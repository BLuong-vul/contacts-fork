...
const DirectMessages = () => {
    ...
    useEffect(() => {
        const fetchUserId = async () => {
            try {
                ...
                const response = await fetch(`https://contacts-5min.onrender.com/user/info`, {
                    method: 'GET',
                    headers: {
    ...
    useEffect(() => {
        if (userId && otherId) {
            const socket = new SockJS('https://contacts-5min.onrender.com/ws');
    ...
    const handleUsernameSubmit = async (event) => {
        ...
        if (inputUsername.trim() !== '') {
            ...
            try {
                const response = await fetch(`https://contacts-5min.onrender.com/user/id/${inputUsername}`);
